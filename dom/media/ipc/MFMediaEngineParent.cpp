...
void MFMediaEngineParent::NotifyError(MF_MEDIA_ENGINE_ERR aError,
  ...
  switch (aError) {
    case MF_MEDIA_ENGINE_ERR_ABORTED: {
      // Engine pipeline abort (e.g. CDM failure). Surface it unless shut down
      // or a hardware reset is already in progress.
    ...
    case MF_MEDIA_ENGINE_ERR_NETWORK:
      // We ignore this because we fetch data by ourselves.
      return;
